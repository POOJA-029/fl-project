...
def calculate_demographic_parity(y_pred, protected_attr):
    ...
    It checks the difference in positive prediction rates between the two groups.
    If closer to 0, it means better fairness.
    """
...
def calculate_equal_opportunity(y_true, y_pred, protected_attr):
    ...
    True Positive Rate (TPR) difference between two groups.
    """
...
def apply_reweighing(X, y, protected_attr):
    """
    A simple Data Resampling / Reweighing simulation for Mitigation.
    We just oversample the minority intersection group to balance out the training set.
    """
    # Combining X, y, protected to resample
    # Find combinations
    ...
    # Find max count to oversample others
...
def get_fair_dataloader(X, y, protected_attr, batch_size=16):
    """
    Returns a DataLoader where the dataset has been reweighed/resampled for Fairness mitigating.
    """
