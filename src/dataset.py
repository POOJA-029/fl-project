...
class FederatedDataset(Dataset):
    def __init__(self, X, y):
        ...
        # Convert y to (N, 1) if it's not and make sure it's float for BCEWithLogitsLoss
...
def load_domain_data(domain="Healthcare"):
    ...
    if domain == "Healthcare":
        ...
        # Load Credit-g (Credit Default / Approval)
        # Target: class (good/bad). Protected: age or personal_status
        data = fetch_openml(data_id=31, as_frame=True, parser='auto')
        df = data.frame
        df = df.dropna()
        
        # Target: bad=1 (high risk/fraud), good=0
        df['target'] = (df['class'] == 'bad').astype(int)
        
        # Protected: age > 30 (just a sample split for fairness)
        ...
        # Protected: sex (Male=1, Female=0)
        df['protected_group'] = (df['sex'] == 'Male').astype(int)
        
        df = df.drop(columns=['class'])
        target_col = 'target'
        protected_col = 'protected_group'
        # Limit rows for faster simulation in sandboxes
    ...
    # Scale all features (Neural networks prefer scaled inputs)
...
def partition_data_for_clients(X, y, protected, num_clients=3):
    """
    Splits the data into partitions to simulate multiple hospitals/banks (Federated Learning clients).
    """
    partitions = []
